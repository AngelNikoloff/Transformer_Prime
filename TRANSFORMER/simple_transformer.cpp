...
namespace nm5 {
    ...
    struct fmat
    {
        ...
        fmat() {}
        fmat(int r, int c, double fill = 0.0) : rows(r), cols(c), data(r* c, fill) {}

        inline double& at(int r, int c) { return data[r * cols + c]; }
        inline double  at(int r, int c) const { return data[r * cols + c]; }
        inline double* row_ptr(int r) { return data.data() + r * cols; }
        inline const double* row_ptr(int r) const { return data.data() + r * cols; }
        void zero() { std::fill(data.begin(), data.end(), 0.0); }
        void resize(int r, int c, double fill = 0.0) { rows = r; cols = c; data.assign(r * c, fill); }
    ...
    struct AdamW
    {
        ...
        AdamW() {}
        AdamW(double b1, double b2, double e, double wd) : beta1(b1), beta2(b2), eps(e), weight_decay(wd) {}

        void step(double* w, const double* grad, int n, double lr, int batch_size = 1)
        {
    ...
    inline avec Relu(const avec& v, double shift = 0.0)
    {
        avec r(v.size());
        for (size_t i = 0; i < v.size(); ++i) r[i] = (v[i] > shift) ? v[i] - shift : 0.0;
        return r;
    }
    inline avec Relu_derivative(const avec& v, double shift = 0.0)
    {
        avec r(v.size());
    ...
    inline avec softmax(const avec& x)
    {
        ...
        avec out(x.size()); double s = 0;
        for (size_t i = 0; i < x.size(); i++) { out[i] = std::exp(x[i] - mx); s += out[i]; }
        for (size_t i = 0; i < x.size(); i++) out[i] /= s;
        return out;
    }
    inline double cross_entropy_loss(const fmat& logits, const ivec& targets)
    {
        double loss = 0.0; int T = logits.rows;
        for (int t = 0; t < T; t++)
        {
    ...
    inline double dot_avx(const double* a, const double* b, int n)
    {
    ...
    public:
        ...
        void init(int _in, int _out, bool relu = false)
        {
            ...
            std::normal_distribution<double> dist(0.0, 0.1); //000 
            //std::normal_distribution<double> dist(0.0, 0.02); // 0.02 for mha head more good ???
        ...
        fmat forward(const fmat& x)
        {
            ...
            for (int t = 0; t < T; ++t)
            {
                const double* xrow = x.row_ptr(t);
                double* yrow = y.row_ptr(t);
                for (int j = 0; j < out_dim; ++j)
                {
                    const double* wrow = W.row_ptr(j);
                    ...
                    //================ if avx supported
                    double s = b[j] + dot_avx(wrow, xrow, in_dim);
        ...
        fmat backward(const fmat& dY)
        {
            ...
            for (int t = 0; t < T; ++t)
            {
                const double* drow = dY_eff.row_ptr(t);
                double* dxrow = dX.row_ptr(t);
                for (int j = 0; j < out_dim; ++j)
                {
                    double d = drow[j];
                    if (d == 0.0) continue;
            ...
            for (int j = 0; j < out_dim; ++j)
            {
                double* dwrow = dW.row_ptr(j);
                double dbi = 0.0;
                for (int t = 0; t < T; ++t)
                {
                    double d = dY_eff.at(t, j);
                    if (d == 0.0) continue;
                    dbi += d;
                    const double* xrow = last_x.row_ptr(t);
    ...
    public:
        ...
        void update(double lr, int bs = 1)
        {
    ...
    public:
        ...
        avec forward(const avec& x)
        {
            ...
            double var = 0.0; for (int i = 0; i < d; i++) var += (x[i] - mean) * (x[i] - mean);
            double si = 1.0 / std::sqrt(var / d + 1e-5);
        ...
        {
            int d = lx.size();
            double mean = std::accumulate(lx.begin(), lx.end(), 0.0) / d;
            double var = 0.0; for (int i = 0; i < d; i++) var += (lx[i] - mean) * (lx[i] - mean);
            double si = 1.0 / std::sqrt(var / d + 1e-5);
    ...
    public:
        ...
        void apply_rope(const double* in, double* out, int D, int pos)
        {
            int half = D / 2;
            for (int d = 0; d < half; ++d)
            {
                double theta = pos / std::pow(10000.0, 2.0 * d / D);
        ...
        void apply_rope_bwd(const double* dout, double* dx, int D, int pos)
        {
            int half = D / 2;
            for (int d = 0; d < half; ++d)
            {
        ...
        fmat forward(const fmat& x)
        {
            ...
            for (int i = 0; i < T; ++i)
            {
                const double* qi = last_q.row_ptr(i);
                avec scores(T, -1e9);
                for (int j = 0; j <= i; ++j)
                {
                    double dot = 0.0;
                    const double* kj = last_k.row_ptr(j);
                    for (int d = 0; d < D; ++d) dot += qi[d] * kj[d];
                    scores[j] = dot * scale;
                }

                double max_s = *std::max_element(scores.begin(), scores.begin() + i + 1);
                double sum_e = 0.0;
                ...
                double* out_i = attn_out.row_ptr(i);
                for (int j = 0; j <= i; ++j)
                {
                    double a = scores[j];
                    const double* vj = last_v.row_ptr(j);
        ...
        fmat backward(const fmat& dout)
        {
            ...
            for (int i = 0; i < T; ++i)
            {
                const double* dout_i = dout.row_ptr(i);
                avec dattn(T, 0.0);
                for (int j = 0; j <= i; ++j)
                {
                    double a = last_attn.at(i, j);
                    const double* vj = last_v.row_ptr(j);
                    double* dvj = dv.row_ptr(j);
                    for (int d = 0; d < D; ++d)
                    {
                ...
                double* dq_i = dq.row_ptr(i);
                for (int j = 0; j <= i; ++j)
                {
                    double dsj = ds[j] * scale;
                    const double* kj = last_k.row_ptr(j);
                    const double* qi = last_q.row_ptr(i);
                    double* dk_j = dk.row_ptr(j);
        ...
        void update(double lr, int batch_size = 1) { Wq.update(lr, batch_size); Wk.update(lr, batch_size); Wv.update(lr, batch_size); }
    ...
    public:
        ...
        MultiHeadAttention() {}
        void init(int _embed_size, int _num_heads = 1, double = 1.0)
        {
        ...
        void update(double lr, int batch_size = 1) { for (auto& h : heads) h.update(lr, batch_size); Wo.update(lr, batch_size); }
    ...
    public:
        ...
        void update(double lr, int bs = 1) { ffn.update(lr, bs); norm1.update(lr, bs); norm2.update(lr, bs); att.update(lr, bs); }
    ...
    public:
        ...
        void init(int vocab_size, int embed_dim, int = 1)
        {
            ...
            std::mt19937 rng(42); std::uniform_real_distribution<double> dist(-0.1, 0.1);
        ...
        void backward(const fmat& errors)
        {
            ...
            {
                double* g = vocab_grad.row_ptr(tokens[t]); const double* e = errors.row_ptr(t); for (int j = 0; j < dim; ++j) g[j] += e[j];
            }
        }
    ...
    public:
        ...
        {
            fmat lg = forward(ctx); const double* last = lg.row_ptr(lg.rows - 1); int best = 0;
...
#pragma omp parallel for schedule(static) reduction(+:step_loss) num_threads(nthreads)
            ...
            std::cout << "Done. Total: " << std::chrono::duration<double>(std::chrono::steady_clock::now() - t0).count() << "s\n";
        }
        ...
        void test()
        {
            ...
            std::cout << "\"\n";
            std::mt19937 rng(999); double test_loss = 0.0;
            for (int i = 0; i < 10; i++)
            {
