...
namespace nm7 {
    ...
    struct ConfigMinimal // 370
    {
        ...
        float dropout_AttHead = 0.0; // 0.1 // not implemented
        float dropout_AttMultiHead = 0.0; // 0.1
...
//================ if avx supported
    ...
    {
    public:
        float p;          // dropout probability (0.0 = no dropout)
        std::vector<int> mask;
        std::mt19937 rng;
        std::uniform_real_distribution<float> dist;

        // Dropout(atype prob = 0.1, uint32_t seed = 1337) : p(prob), rng(seed), dist(0.0f, 1.0f) {}
        Dropout(float prob = 0.1) : p(prob), dist(0.0f, 1.0f)
        {
            uint32_t seed = 1337;
            rng.seed(seed);
        }
        void _init(float _dropout = 0.0)
        {
    ...
    public:
        ...
        fmat forward(const fmat& x)
        {
            ...
            for (int i = 0; i < T; ++i)
            {
                ...
                //a dropout must be here?

                atype* out_i = attn_out.row_ptr(i);
                for (int j = 0; j <= i; ++j)
                {
                    //atype a = scores[j]; //000
        ...
        fmat backward(const fmat& dout)
        {
            ...
            for (int i = 0; i < T; ++i)
            {
                // Backward through value weighted sum
        ...
        void reduce_from(const TAttentionHeadRoPE& o) { Wq.reduce_from(o.Wq); Wk.reduce_from(o.Wk); Wv.reduce_from(o.Wv); }
        void broadcast_to(TAttentionHeadRoPE& o) const { Wq.broadcast_to(o.Wq); Wk.broadcast_to(o.Wk); Wv.broadcast_to(o.Wv); }
    };
    ...
    public:
        ...
        std::vector<TAttentionHeadRoPE> heads;
        FLinear Wo;
        fmat last_concat;
        Dropout dropout;
        ...
        fmat forward(const fmat& x)
        {
            ...
            if (con.dropout_AttMultiHead && con.train) dropout.forward(out);

            return out;
        }
        // fmat backward(const fmat& dout)
        fmat backward(fmat& dout)
        {
            if (con.dropout_AttMultiHead && con.train) dropout.backward(dout);
